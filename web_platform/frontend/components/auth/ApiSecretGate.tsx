...
export function ApiSecretGate({ children }: ApiSecretGateProps) {
    ...
    const validateStoredSecret = async (storedSecret: string) => {
        try {
            const response = await fetch(`${API_CONFIG.baseURL}/api/system/validate-secret`, {
                method: 'POST',
                headers: {
        ...
        <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
            <Card className="w-full max-w-md p-8 bg-zinc-900 border-zinc-800">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label htmlFor="api-secret" className="block text-sm font-medium text-zinc-300 mb-2">
                            API Secret Key
                        </label>
                        <Input
                            id="api-secret"
                ...
                <div className="mt-8 p-4 bg-blue-500/5 border border-blue-500/10 rounded-lg">
                    <div className="flex items-start gap-2">
                        ...
                        <div className="text-xs text-zinc-400">
                            ...
                            <p>The API secret is stored locally in your browser and sent with every request to verify access to the backend API.</p>
                        </div>
                    </div>
                </div>
